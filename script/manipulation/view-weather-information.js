...
import {
  setWeatherDetails,
  setWeatherDetailsForCurrentLocation,
} from "./show-details-of-weather.js";
...
export function showGeneralInformation(jsonData) {
  ...
  /* Show information in case of forecast weather data request */
    ...
    if (timePicker.value === "") {
      ...
      changeTemperature = () =>
        changeTempType(
          jsonData.forecast.forecastday[0].day.avgtemp_f,
          jsonData.forecast.forecastday[0].day.avgtemp_c
        );
      tempEL.addEventListener("click", changeTemperature);
      tempELHasEvenListener = true;
    ...
    // when user give a day from now and 5 days ahead with 'specific time'
      ...
      weatherConditionInfo.textContent =
        jsonData.forecast.forecastday[0].hour[0].condition.text;
      weatherConditionLogo.src =
        jsonData.forecast.forecastday[0].hour[0].condition.icon;
      clockInfo.textContent = showRightTime(
        jsonData.forecast.forecastday[0].hour[0].time
      );
      temperatureDegree.textContent = Math.round(
        jsonData.forecast.forecastday[0].hour[0].feelslike_c
      );

      // change the temp type
      changeTemperature = () =>
        changeTempType(
          jsonData.forecast.forecastday[0].hour[0].feelslike_f,
          jsonData.forecast.forecastday[0].hour[0].feelslike_c
        );
      tempEL.addEventListener("click", changeTemperature);
      tempELHasEvenListener = true;
